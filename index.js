...
const watchCss = async () => {
  ...
  try {
    ...
    const path = ['~', 'extensions', NAME, 'style.css'].join('/');
...
const applyAdditionalMatchingSourcesVisibility = (enabled) => {
  ...
  for (const header of document.querySelectorAll(
    '.stwid--editor .userSettingsInnerExpandable, .stwid--editor .inline-drawer.stwid--ams',
  )) {
...
export const jumpToEntry = async (name, uid) => {
  ...
  if (currentEditor?.name !== name || String(currentEditor?.uid) !== String(uid)) {
    entryDom.click();
  }
  return true;
};
