...
[CmdletBinding()]
param(
    [string]$GroupDir = "tasks/js-refactoring/ready-for-implementation/group-6-entry-table-bulk",
...
if ($branch -eq 'main' -or $branch -eq 'master' -or $branch -eq 'dev') {
    ...
    Write-Warning "Recommended: git checkout -b chore/cline-js-refactor-g6"
...
$commitMsg = "refactor(entry-manager): apply js refactoring group 6 (entry-table-bulk) via cline`n`nFiles refactored:`n$bodyLines`n"
