...
if ($branch -eq 'main' -or $branch -eq 'master' -or $branch -eq 'dev') {
    Write-Warning "You are on '$branch'. This script will update multiple task files."
    Write-Warning "Recommended: git checkout -b chore/js-refactoring-cline-batch"
...
Write-Host "=============================================================================="
Read-Host "This will run one separate Cline session per queued file. Press Enter to start, or Ctrl+C to abort"
...
while ($true) {
    ...
    $prompt = @"
Run the js-refactoring skill instructions for exactly one file.
...
Requirements:
- Process ONLY one queued file in this Cline session.
...
Write-Host "=============================================================================="
Write-Host ""
Write-Host "Review the queue and reports with: git diff -- tasks/js-refactoring/"
