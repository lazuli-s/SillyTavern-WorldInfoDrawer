...
[CmdletBinding()]
param(
    [string]$GroupDir = "tasks/js-refactoring/ready-for-implementation/group-5-drawer-editor",
...
if ($branch -eq 'main' -or $branch -eq 'master' -or $branch -eq 'dev') {
    ...
    Write-Warning "Recommended: git checkout -b chore/cline-js-refactor-g5"
...
$commitMsg = "refactor(drawer): apply js refactoring group 5 (drawer-editor) via cline`n`nFiles refactored:`n$bodyLines`n"
