...
[CmdletBinding()]
param(
    [string]$GroupDir = "tasks/js-refactoring/ready-for-implementation/group-4-browser-tabs",
...
if ($branch -eq 'main' -or $branch -eq 'master' -or $branch -eq 'dev') {
    ...
    Write-Warning "Recommended: git checkout -b chore/cline-js-refactor-g4"
...
$commitMsg = "refactor(browser-tabs): apply js refactoring group 4 (browser-tabs) via cline`n`nFiles refactored:`n$bodyLines`n"
