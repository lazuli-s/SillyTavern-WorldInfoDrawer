...
[CmdletBinding()]
param(
    [string]$GroupDir = "tasks/js-refactoring/ready-for-implementation/group-3-book-list-folders",
...
if ($branch -eq 'main' -or $branch -eq 'master' -or $branch -eq 'dev') {
    ...
    Write-Warning "Recommended: git checkout -b chore/cline-js-refactor-g3"
...
$commitMsg = "refactor(book-browser): apply js refactoring group 3 (book-list-folders) via cline`n`nFiles refactored:`n$bodyLines`n"
