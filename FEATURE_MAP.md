...
- Visibility thin-container row groups Book Visibility trigger + chips + Order Helper toggle in one place → src/listPanel.filterBar.js, src/drawer.js, style.css
...
- Bulk edit row structure (field containers: Select, State, Strategy, Position, Depth, Order, Recursion, Budget, Probability, Sticky, Cooldown, Delay, Apply All Changes) → src/orderHelperRender.actionBar.js
- Dirty indicator on bulk edit Apply buttons (amber highlight when inputs changed, clears on successful apply) → src/orderHelperRender.actionBar.js, style.css
- Apply All Changes container (runs all dirty containers in sequence, skips clean ones) → src/orderHelperRender.actionBar.js
