...
const createSelectionStateHelpers = ({ listPanelState, runtime }) => {
  const selectEnd = () => {
    ...
    [...runtime.dom.books.querySelectorAll('.stwid--entry.stwid--state-selected')].forEach(
      (selectedEntry) => {
    ...
    [...runtime.dom.books.querySelectorAll(`.stwid--book.${DROP_TARGET_CLASS}`)].forEach(
      (selectedEntry) => {
  ...
  const keepOnlyFailedSelection = (failedUids) => {
    ...
    listPanelState.selectList = listPanelState.selectList.filter((uid) =>
      failedUidSet.has(String(uid)),
    );
...
const restoreBookSnapshot = (book, snapshot) => {
...
const transferSelectedEntries = async (
  srcBook,
  dstBook,
  selectedUids,
  isCopy,
  hasConcurrentBookUpdate,
  runtime,
) => {
  const result = {
...
const finalizeEntryTransfer = async (
  ...
  const saveBook = async (bookName, bookData, saveTarget) => {
    await runtime.saveWorldInfo(bookName, bookData, true);
    if (typeof runtime.updateWIChange === 'function') {
      runtime.updateWIChange(bookName, bookData);
    } else {
      console.warn(`[STWID] runtime.updateWIChange is unavailable for ${saveTarget} save.`);
    }
  };

  const shouldSaveSourceBook = !isCopy && sourceBookName !== targetBookName && result.hasSrcChanges;
  let destinationSaved = false;

  try {
    if (result.hasDstChanges) {
      await saveBook(targetBookName, dstBook, 'destination');
      destinationSaved = true;
    }

    if (shouldSaveSourceBook) {
      await saveBook(sourceBookName, srcBook, 'source');
    }
  } catch (error) {
    if (previousSrcBook) {
      restoreBookSnapshot(srcBook, previousSrcBook);
    }
    if (destinationSaved && previousDstBook && previousSrcBook) {
      try {
  ...
  if (!isCopy && result.failedUids.length > 0) {
    ...
    toastr.warning(
      `Move partially completed. ${succeeded} moved, ${result.failedUids.length} failed. Failed entries remain selected.`,
    );
    return;
  }
...
const createEntryTransferHandlers = ({
  listPanelState,
  runtime,
  keepOnlyFailedSelection,
  selectEnd,
  ...
  const moveOrCopySelectedEntriesToBook = async (targetBookName, isCopy) => {
    ...
    try {
      ...
      const previousSrcBook =
        !isCopy && sourceBookName !== targetBookName ? structuredClone(srcBook) : null;
      const previousDstBook = structuredClone(dstBook);
...
const createSelectionDnDSlice = ({ listPanelState, runtime, handleDraggedBookMoveOrCopy }) => {
  const { keepOnlyFailedSelection, selectAdd, selectEnd, selectRemove } =
    createSelectionStateHelpers({
      listPanelState,
      runtime,
    });
