...
const waitForImportedBooksToStabilize = async (getWorldNames, updatePromise) => {
  ...
  while (Date.now() < deadline) {
    ...
    const unchanged =
      nextSnapshot.length === lastSnapshot.length &&
...
const matchImportedBookNames = (expectedBookNames, beforeNames, afterNames) => {
  ...
  const attributedNames = expectedBookNames.length
    ? newNames.filter(
        (name) =>
          expectedBookNames.includes(name) ||
...
const moveImportedBooksIntoFolder = async (
  attributedNames,
  folderName,
  menuActions,
  unmatchedNames,
...
const createBookInFolder = async ({
  ...
  if (!newBookData || typeof newBookData !== 'object') {
    ...
    toastr.warning(
      `Created "${finalName}", but it could not be placed into folder "${folderName}". You can move it manually.`,
    );
    await refreshList?.();
    return finalName;
  }
...
const importFolderAction = async ({ folderName, menuActions }) => {
  ...
  try {
    ...
    if (!expectedBookNames.length) {
      toastr.warning(
        'This import format cannot be attributed safely. New books will not be auto-moved into the folder.',
      );
      return;
    }
    const hasUpdate = await waitForImportedBooksToStabilize(
      menuActions.getWorldNames,
      updatePromise,
    );
    if (!hasUpdate) {
      console.warn(
        `[STWID] Folder import timed out waiting for WORLDINFO_UPDATED for folder "${folderName}".`,
      );
    ...
    const { attributedNames, unmatchedNames } = matchImportedBookNames(
      expectedBookNames,
      beforeNames,
      afterNames,
    );
    ...
    if (!attributedNames.length) {
      toastr.warning(
        'Import finished, but new books could not be confidently identified. No books were moved into the folder.',
      );
      return;
    }
...
const entryManagerFolderAction = ({ folderName, menuActions }) => {
  const visibleScope =
    menuActions.getBookVisibilityScope?.() ?? menuActions.getSelectedWorldInfo?.() ?? [];
  const visibleLookup = new Set(visibleScope);
  const bookNames = getFolderBookNames(menuActions.cache, folderName).filter((name) =>
    visibleLookup.has(name),
  );
