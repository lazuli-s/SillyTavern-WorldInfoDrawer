...
const createFoldersViewSlice = ({ listPanelState, runtime, selectionDnDSlice }) => {
  let bulkFolderCollapsedIntent = null;
  let folderGroupingVisible = true;

  const hasExpandedFolders = () =>
    listPanelState.getFolderDomValues().some((folderDom) => {
  ...
  const createFolderDomForList = (folderName) => {
    const folderDom = createFolderDom({
      folderName,
      onToggle: () => {
        const currentFolderDom = listPanelState.getFolderDom(folderName);
        const books = currentFolderDom?.books;
        if (!books) return;
  ...
  const ensureFolderDom = (folderName, parent = runtime.dom.books) => {
    ...
    const initialCollapsed =
      bulkFolderCollapsedIntent ?? listPanelState.getFolderCollapseState(folderName) ?? true;
  ...
  const updateFolderVisibility = ({ isBookDomFilteredOut }) => {
    ...
    for (const folderDom of listPanelState.getFolderDomValues()) {
      const bookRows = Array.from(folderDom.books.children).filter(
        (child) => child instanceof HTMLElement && child.classList.contains(BOOK_ROW_CLASS),
      );
      const hasVisibleBooks = bookRows.some((child) => !isBookDomFilteredOut(child));
      folderDom.root.hidden = bookRows.length > 0 && !hasVisibleBooks;
    }
  };
