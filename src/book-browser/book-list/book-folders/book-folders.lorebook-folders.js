...
const getVisibleFolderBookNames = (cache, folderName) =>
  getFolderBookNames(cache, folderName).filter((name) => {
...
const getFolderActiveState = (cache, selected, folderName, bookNamesOverride = null) => {
  const bookNames = Array.isArray(bookNamesOverride)
    ? bookNamesOverride
    : getFolderBookNames(cache, folderName);
  if (!bookNames.length) {
