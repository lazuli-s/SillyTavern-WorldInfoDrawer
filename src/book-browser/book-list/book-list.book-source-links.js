...
const getBookSourceRuntimeContext = () => {
  ...
  const eventTypes =
    context?.eventTypes && typeof context.eventTypes === 'object'
      ? context.eventTypes
      : context?.event_types && typeof context.event_types === 'object'
        ? context.event_types
        : {};
  return {
    chatMetadata:
      context?.chatMetadata && typeof context.chatMetadata === 'object' ? context.chatMetadata : {},
...
const collectCharacterBooks = (runtime) => {
  ...
  if (runtime.groupId) {
    const activeGroup = runtime.groups.find(
      (group) => String(group?.id) === String(runtime.groupId),
    );
    const members = Array.isArray(activeGroup?.members) ? activeGroup.members : [];
    const { characterByAvatar, charactersByName } = indexCharactersByAvatarAndName(
      runtime.characters,
    );
  ...
  if (
    runtime.characterId !== undefined &&
    runtime.characterId !== null &&
    runtime.characters[runtime.characterId]
  ) {
    addCharacterLinkedBooks(
      characterBooks,
      runtime.characters[runtime.characterId],
      runtime.characterId,
    );
...
export const initBookSourceLinks = ({ getListPanelApi }) => {
  ...
  const buildLorebookSourceLinks = () => {
    ...
    for (const [worldName, characterNameSet] of characterBooks.entries()) {
      ...
      linksByBook[worldName].characterNames = [...characterNameSet].sort((a, b) =>
        a.localeCompare(b),
      );
    }
