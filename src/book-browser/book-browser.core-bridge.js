const CORE_UI_ACTION_SELECTORS = Object.freeze({
  duplicateBook: Object.freeze(['#world_duplicate', '[id="world_duplicate"]']),
  deleteBook: Object.freeze(['#world_popup_delete', '[id="world_popup_delete"]']),
...
const setSelectedBookInCoreUi = async (
  bookName,
  { waitForWorldInfoUpdate = null, delay, worldEditorSelectSelector = '#world_editor_select' },
) => {
  const worldEditorSelect = document.querySelector(worldEditorSelectSelector);
  if (!worldEditorSelect) return false;
  const matchedBookOption = [...worldEditorSelect.children].find(
...
const clickCoreUiAction = async (
  possibleSelectors,
  { timeoutMs = DEFAULT_DOM_WAIT_TIMEOUT_MS } = {},
) => {
...
export { CORE_UI_ACTION_SELECTORS, clickCoreUiAction, setSelectedBookInCoreUi, waitForDom };
