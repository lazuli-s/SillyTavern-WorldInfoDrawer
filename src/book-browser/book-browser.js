...
import { createFilterBarSlice } from './browser-tabs/browser-tabs.js';
...
const setAllFoldersCollapsed = (isCollapsed) =>
  foldersViewSlice?.setAllFoldersCollapsed(isCollapsed);
const setFolderGroupingVisibility = (enabled) =>
  foldersViewSlice?.setFolderGroupingVisibility(enabled);
const setEntryManagerToggleVisibility = (enabled) =>
  filterBarSlice?.setEntryManagerToggleVisibility(enabled);
...
const getBookVisibilityScope = (selectedNames = null) =>
  filterBarSlice?.getBookVisibilityScope(selectedNames) ?? [];
...
const renderBookSourceLinks = (sourceLinksContainer, links = null) => {
  ...
  sourceLinksContainer.classList.toggle(
    'stwid--state-empty',
    sourceLinksContainer.childElementCount === 0,
  );
};
...
const clearBookSortPreferences = async () => {
  ...
  if (failedBooks.length) {
    toastr.error(
      `Could not clear sort preferences for ${failedBooks.length} book${failedBooks.length === 1 ? '' : 's'}.`,
    );
...
const handleDraggedBookMoveOrCopy = async (
  draggedName,
  targetFolder,
  isCopy,
  { skipIfSameFolder = true } = {},
) => {
  if (!isCopy) {
    if (skipIfSameFolder) {
...
const setSelectedBookInCoreUi = (bookName) =>
  setSelectedBookInCoreUiBridge(bookName, {
    waitForWorldInfoUpdate: state.waitForWorldInfoUpdate,
    delay: state.delay,
  });

const clickCoreUiAction = (possibleSelectors, options = {}) =>
  clickCoreUiActionBridge(possibleSelectors, options);
...
const updateFolderActiveToggles = () =>
  foldersViewSlice?.updateFolderActiveToggles({ isBookDomFilteredOut });
...
const initBookBrowser = (options) => {
  if (listPanelInitialized) {
    console.warn(
      '[STWID] initBookBrowser called more than once; resetting previous book browser instance.',
    );
    teardownListPanel();
  }
  state = { ...options };
  ...
  listPanelState.folderMenuActions = {
    ...
    refreshList,
    setBookFolder,
    setBooksActive: (bookNames, isActive) =>
      setFolderBooksActive(bookNames, isActive, state.onWorldInfoChange),
    waitForWorldInfoUpdate: state.waitForWorldInfoUpdate,
  };
