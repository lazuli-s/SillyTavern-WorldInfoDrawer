...
const BOOK_VISIBILITY_OPTION_TOOLTIPS = Object.freeze({
  [BOOK_VISIBILITY_MODES.ALL_BOOKS]:
    'Show all books in the list, including books that are not currently active from any source.',
  [BOOK_VISIBILITY_MODES.ALL_ACTIVE]:
    'Show books active from any source (Global, Chat, Persona, or Character). This shows all books currently added to context by these sources.',
  ...
  [BOOK_VISIBILITY_MODES.CHARACTER]:
    'Include books linked to the current character (or all group members).',
});
...
const createVisibilityMenuButton = () => {
  ...
  visibilityMenuButton.title =
    'Select which book sources are visible in the list and Entry Manager.';
...
const buildVisibilityMenuOptions = ({
  ...
  for (const option of BOOK_VISIBILITY_OPTIONS) {
    ...
    if (
      option.mode !== BOOK_VISIBILITY_MODES.ALL_BOOKS &&
      option.mode !== BOOK_VISIBILITY_MODES.ALL_ACTIVE
    ) {
      const optionCheckbox = document.createElement('i');
      optionCheckbox.classList.add('fa-solid', 'fa-fw', CSS_MULTISELECT_DROPDOWN_OPTION_CHECKBOX);
...
const createVisibilityScopeHelpers = ({
  ...
  const getBookVisibilityFlags = (name, selectedLookup = null) => {
    ...
    const selected =
      selectedLookup instanceof Set ? selectedLookup : new Set(getSelectedWorldInfo());
  ...
  const getBookVisibilityScope = (selectedNames = null) => {
    ...
    return cacheEntries.filter((name) => {
      ...
      return BOOK_VISIBILITY_MULTISELECT_MODES.some(
        (mode) => listPanelState.bookVisibilitySelections.has(mode) && flags[mode],
      );
    });
  };
...
const syncVisibilityMenuOptionState = ({
  ...
  const isActive =
    optionMode === BOOK_VISIBILITY_MODES.ALL_BOOKS
      ? isAllBooks
  ...
  () => {
    ...
    for (const bookName of Object.keys(runtime.cache)) {
      ...
      runtime.cache[bookName].dom.root.classList.toggle(
        ...
        hideByVisibilityFilter,
      );
    }
    if (listPanelState.bookVisibilityMenu) {
      for (const optionEl of listPanelState.bookVisibilityMenu.querySelectorAll(
        `.${CSS_MULTISELECT_DROPDOWN_OPTION}`,
      )) {
...
const createVisibilitySlice = ({
  ...
  const getSelectedWorldInfo = () =>
    runtime.getSelectedWorldInfo ? runtime.getSelectedWorldInfo() : runtime.selected_world_info;
...
export const mountVisibilityTabContent = ({ tabContentsById, visibilityRow }) => {
...
export { BOOK_VISIBILITY_MODES, createVisibilitySlice };
