        ...
        await withApplyButtonLock(applyButton, async()=>{
            ...
            if (collisions.length > 0) {
                const details = collisions
                    .map(({ bookName, collidingUids })=>`in "${bookName}" these UIDs are already used by unselected entries: ${collidingUids.join(', ')}`)
                    .join('; ');
...
export function buildBulkUidSection({
    ...
    applyUid = createApplyButton(
        'Renumber selected entries’ UIDs',
        runApplyUid,
        applyRegistry,
