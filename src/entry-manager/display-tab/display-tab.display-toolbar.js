...
function applyAndPersistColumnVisibility({
  ...
  return (overrides) => {
    ...
    localStorage.setItem(storageKey, JSON.stringify(entryManagerState.columns));
...
function buildColumnCheckboxOptions(
  menu,
  columns,
  entryManagerState,
  columnInputs,
...
function buildColumnVisibilityDropdown({
  ...
  const { columnVisibilityContainer, columnVisibilityWrap, menuWrap, menuButton, menu } =
    createColumnVisibilityDropdownDom();
  const columnInputs = new Map();
  const mainColumnDefaults = Object.fromEntries(
    Object.entries(ENTRY_MANAGER_DEFAULT_COLUMNS).map(([key, value]) => [key, Boolean(value)]),
  );
...
function buildFilterChipDisplay({
  ...
  const refresh = () => {
    const filterConfigs = [
      {
        key: 'strategy',
        allValues: entryManagerState.strategyValues.length
          ? entryManagerState.strategyValues
          : getStrategyValues(),
      },
      {
        key: 'position',
        allValues: entryManagerState.positionValues.length
      ...
      {
        key: 'outlet',
        allValues: entryManagerState.outletValues.length
          ? entryManagerState.outletValues
          : getOutletValues(),
      },
      {
        key: 'automationId',
        allValues: entryManagerState.automationIdValues.length
  ...
  const getFilterValueLabels = (filterKey, selectedValues) => {
    ...
    switch (filterKey) {
      ...
      case 'position':
        ...
        break;
      case 'outlet':
        options = getOutletOptions();
        break;
      case 'automationId':
        options = getAutomationIdOptions();
        break;
      case 'group':
...
function buildClearFilterHandlers({
  ...
  const filterHandlerConfigs = [
    ...
    {
      key: 'automationId',
      getAllValues: getValues.automationId,
  ...
  return Object.fromEntries(
    filterHandlerConfigs.map(({ key, getAllValues, applyFn }) => [
      key,
