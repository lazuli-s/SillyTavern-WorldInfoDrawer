...
const STRATEGY_FALLBACK_OPTIONS = Object.freeze([
  { value: 'constant', label: '🔵' },
  { value: 'normal', label: '🟢' },
  { value: 'vectorized', label: '🔗' },
]);

const POSITION_FALLBACK_OPTIONS = Object.freeze([
  { value: '0', label: '↑Char' },
  { value: '1', label: '↓Char' },
  { value: '5', label: '↑EM' },
  { value: '6', label: '↓EM' },
...
const getStrategyOptions = () => {
  const options = getSelectOptions('#entry_edit_template [name="entryStateSelector"]');
  return options.length > 0 ? options : STRATEGY_FALLBACK_OPTIONS;
};

const getStrategyValues = () => getOptionValues(getStrategyOptions());

const getPositionOptions = () => {
  const options = getSelectOptions('#entry_edit_template [name="position"]');
  return options.length > 0 ? options : POSITION_FALLBACK_OPTIONS;
};
...
const buildCanonicalDefaultColumns = () => {
  ...
  const missingInDefaults = schemaKeys.filter(
    (key) => !Object.hasOwn(ENTRY_MANAGER_DEFAULT_COLUMNS, key),
  );
  const extraInDefaults = defaultKeys.filter((key) => !schemaKeySet.has(key));
  if (missingInDefaults.length > 0 || extraInDefaults.length > 0) {
    console.warn('[STWID entryManagerState] Column schema mismatch detected.', {
      missingInDefaults,
      extraInDefaults,
    });
  }
...
const buildInitialEntryManagerState = ({
  canonicalDefaultColumns,
  recursionValues,
  SORT,
  SORT_DIRECTION,
...
const applyStoredEntryManagerState = ({
  state,
  schemaKeys,
  canonicalDefaultColumns,
  SORT,
  ...
  if (
    Object.values(SORT).includes(storedSortSettings?.sort) &&
    Object.values(SORT_DIRECTION).includes(storedSortSettings?.direction)
  ) {
...
const createEntryManagerState = ({ SORT, SORT_DIRECTION }) => {
  ...
  const state = buildInitialEntryManagerState({
    canonicalDefaultColumns,
    recursionValues,
    SORT,
    SORT_DIRECTION,
  });
  applyStoredEntryManagerState({
    state,
    schemaKeys,
    canonicalDefaultColumns,
    SORT,
