# Refactoring — CSS Audit Low Priority Fixes
...
| DGR-03 | Danger | Element-qualified class selectors (`i.fa-solid…` prefix on class-only rules) |

FMT-04 is by far the largest change — it involves restructuring ~60 selectors across five separate areas using CSS nesting. All other steps are small, targeted fixes.

---

## Findings to Fix

### FMT-01 — Double quotes in attribute selectors
...
### FMT-03 — Duplicate section number 5
...
Fix — rename the context menu header to `5.5)`:
```css
...
**Lines 1979–1980** (inside the `.stwid--orderInputTight` rule):
```css
  padding-left: 5px;
  padding-right: 5px;
```
...
### DGR-03 — Element-qualified class selectors
...
```
Fix — keep only the extension class:
```css
...
### FMT-04 — Nest repeated parent selectors
...
**What nesting does:** Instead of writing `.stwid--folder .stwid--folderHeader { }` and `.stwid--folder .stwid--folderIcon { }` as separate flat rules, they become child rules inside one `.stwid--folder { }` block using `&` syntax. No behavior or visual change — it is a formatting-only restructure.

**Group 5a — `.stwid--folder` (lines ~585–690, ~11 rules)**
...
- Lines 666–676 (`.stwid--folder .stwid--actions .stwid--action` grouped with `.stwid--book .stwid--actions .stwid--action`) — this rule applies to BOTH folder and book, so it cannot go inside `.stwid--folder {}`. Leave it flat.
- Line 613 (`.stwid--folder:not(:has(...)) .stwid--folderIcon::before`) — becomes `&:not(:has(...)) .stwid--folderIcon::before { }` inside the block.
...
.stwid--folder {
  ...
  & .stwid--folderIcon,
  & .stwid--folderActiveToggle { ... }         /* was lines 642–643 */
...
/* leave flat — shared with .stwid--book */
...
**Group 5b — `.stwid--books .stwid--book` (lines ~695–752, ~10 rules)**
...
**Group 5c — `.stwid--books .stwid--entry` (lines ~757–848, ~16 rules)**
...
**Group 5d — `.stwid--blocker .stwid--listDropdownMenu .stwid--listDropdownItem` (lines ~1188–1229, ~5 rules)**
...
Note: Lines 1206 and 1212 use `.stwid--listDropdownMenu .stwid--listDropdownItem` without the `.stwid--blocker` prefix — these must remain outside the new nesting block.
...
Work through steps in this order. Steps 1–4 are small, safe, and independent — do them first. Step 5 is the only complex step and should be done last, one sub-group at a time.

### Step 1 — FMT-01: Change double quotes to single quotes

- [ ] Line 533: Change `[data-selected="true"]` to `[data-selected='true']`
- [ ] Line 551: Change `[data-selected="true"]` to `[data-selected='true']`

### Step 2 — FMT-03: Renumber duplicate section 5 header

- [ ] Line 1112: Change `/* 5) Context Menu + Modals */` to `/* 5.5) Context Menu + Modals */`

### Step 3 — DGR-03: Remove element qualifiers from class selectors

- [ ] Line 192: Replace `i.fa-solid.fa-fw.fa-circle-question.stwid--thinContainerLabelHint` with `.stwid--thinContainerLabelHint`
- [ ] Line 1502: Replace `i.fa-solid.fa-fw.fa-circle-question.stwid--bulkEditLabelHint` with `.stwid--bulkEditLabelHint`

### Step 4 — PROP-01: Replace separate padding properties with shorthand

- [ ] Lines 1545–1546: Replace `padding-left: 5px; padding-right: 5px;` with `padding-inline: 5px;`
- [ ] Lines 1979–1980: Replace `padding-left: 5px; padding-right: 5px;` with `padding-inline: 5px;`

### Step 5 — FMT-04: Nest repeated parent selectors
...
- [ ] **5a** — Nest `.stwid--folder` sub-rules (lines ~585–690). Leave the shared action affordance rules (lines 666–676) flat outside the block.
- [ ] **5b** — Nest `.stwid--books .stwid--book` sub-rules (lines ~695–752) inside the existing `.stwid--books .stwid--book {}` block.
- [ ] **5c** — Nest `.stwid--books .stwid--entry` sub-rules (lines ~757–848) inside the existing `.stwid--books .stwid--entry {}` block.
- [ ] **5d** — Nest `.stwid--blocker .stwid--listDropdownMenu .stwid--listDropdownItem` sub-rules (lines ~1188–1229). Leave the two prefix-less `.stwid--listDropdownMenu .stwid--listDropdownItem` rules flat.
- [ ] **5e** — Nest `.stwid--orderTable tr td` sub-rules (lines ~1740–1995). Merge the two separate `tr td` blocks into one nested block.
