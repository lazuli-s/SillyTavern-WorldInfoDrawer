...
| NAME-06 | Give each class a name that is unique enough on its own that no parent scope is needed to distinguish it from similar classes elsewhere. If two classes in different components share a name but have different styles, rename the less-established one rather than relying on parent scoping to disambiguate. |
...
| FMT-08 | Group unrelated style sections with a section comment. Example: `/* 1) Drawer */`. Required when the stylesheet covers multiple distinct UI areas. |
...
CSS transitions or animations. Also triggers automatically during any code review that
touches CSS files. For motion-sensitivity accessibility rules, see `css-accessibility` (ACC-02).
...
| ANIM-02 | Never use bare shorthand `transition: <duration>` without naming the property. `transition: opacity 200ms ease` ✓ — `transition: 200ms` ✗. Bare shorthand animates ALL changed properties including invisible layout ones. |
...
Authoring companion and review tool enforcing CSS-specific accessibility rules for this
SillyTavern extension. Covers focus styles and motion sensitivity. Does NOT replace the
HTML/ARIA accessibility concerns handled by `fixing-accessibility`. Use when writing new
CSS that affects interactive elements, or automatically during any code review that touches
CSS files.
...
| ID | Rule |
| --- | --- |
| ACC-01 | Never remove a focus indicator without providing a visible replacement. `outline: none` or `outline: 0` alone is a violation. Always pair with a visible `:focus-visible` alternative (e.g. `box-shadow`, `border`, custom outline). |
...
- Delete `.agents/skills/fixing-accessibility/` (replaced by `css-accessibility`)
- Open a separate refactoring task for the existing `style.css` transition violations
  (bare `transition: 200ms`, `margin-bottom` transition, `max-height` transition)
...
- [ ] `css-rules` skill created (includes naming rules — NAME-01 through NAME-06)
- [ ] `css-animation` skill created
- [ ] `css-accessibility` skill created
- [ ] `.agents/skills/fixing-accessibility/` deleted (replaced by `css-accessibility`)
