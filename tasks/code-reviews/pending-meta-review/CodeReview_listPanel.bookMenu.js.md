...
- **Implementation Checklist:**
  - [ ] Investigate `waitForWorldInfoUpdate` behavior in `src/wiUpdateHandler.js`
  - [ ] If it can resolve with the book name, use that directly
  - [ ] Otherwise, add event-based detection using `eventSource.once(event_types.WORLDINFO_UPDATED, handler)`
  - [ ] Add proper error message if timeout occurs
