...
- [x] Convert `entrySearchCache`, `collapseStates`, `folderCollapseStates`, and `folderDoms` to `Map` or `Object.create(null)`.
- [x] Update getters/setters (`getFolderDom`, `setFolderDom`, etc.) to use the new map API.
- [x] Update call sites that use `Object.keys` / `Object.values` to iterate through map keys/values.
- [x] Add key normalization/validation for book/folder names before insert.
...
- What changed
  - Files changed: `src/listPanel.state.js`, `src/listPanel.foldersView.js`
  - Replaced object-backed state maps with `Object.create(null)` maps for `entrySearchCache`, `collapseStates`, `folderCollapseStates`, and `folderDoms`.
  - Added key normalization/validation helpers and routed state getters/setters through validated keys before reads/writes.
  - Added `getFolderCollapseState` / `setFolderCollapseState` and updated folder view lookup to use the accessor API.

- Risks / Side effects
  - Empty-string or non-string book/folder keys are now ignored by state writes. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Create or rename a book/folder with normal names and confirm collapse state and folder rendering still behave exactly as before.
  - Legacy persisted folder collapse values are normalized to booleans at hydrate time. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Collapse/expand multiple folders, reload the page, and confirm each folder restores to the same collapsed state.
...
- [x] Add a selection reset step during `refreshList()` (or inside `clearCacheBooks()` if that's the chosen hook).
- [x] Ensure the selection toast is cleared when selection resets.
- [x] Verify delete/drag operations no longer reference stale selections after a refresh.
...
- What changed
  - Files changed: `src/listPanel.state.js`, `src/listPanel.js`
  - Updated `clearCacheBooks` to clear selection memory during cache rebuilds.
  - Wired `refreshList` cache clears to pass the runtime `clearToast` handler so the selection toast is dismissed with the reset.
  - Kept selection behavior unchanged outside refresh/reload flow.

- Risks / Side effects
  - Active multi-selection is now intentionally cleared after each full list refresh. (probability: ❗)
      - **🟥 MANUAL CHECK**: [ ] Select multiple entries, trigger Refresh, and confirm selection highlight/toast clears immediately.
  - Delete/drag actions after refresh now operate only on newly selected entries. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Select entries, refresh the list, then press Delete or drag; confirm nothing from the old selection is affected.
