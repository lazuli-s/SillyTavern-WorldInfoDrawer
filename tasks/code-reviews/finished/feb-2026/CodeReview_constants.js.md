...
- [x] Update the JSDoc comments for `SORT_DIRECTION.ASCENDING` and `SORT_DIRECTION.DESCENDING` to correctly describe direction semantics.
...
- What changed
  - Files changed: `src/constants.js`
  - Replaced incorrect `SORT_DIRECTION` docstrings with direction-accurate semantics.
  - Kept runtime values unchanged (`'ascending'` / `'descending'`), so behavior is unchanged.

- Risks / Side effects
  - Documentation-only change; no runtime side effects expected. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Open any sort menu and confirm sort direction behavior is unchanged.
...
- [x] Import `ORDER_HELPER_RECURSION_OPTIONS` into `src/orderHelperState.js`.
- [x] Replace the hardcoded `recursionValues = [...]` array with `ORDER_HELPER_RECURSION_OPTIONS.map(({ value }) => value)`.
- [x] Keep `state.filters.recursion` initialized from that derived list.
...
- What changed
  - Files changed: `src/orderHelperState.js`
  - Replaced hardcoded recursion values with `ORDER_HELPER_RECURSION_OPTIONS.map(({ value }) => value)`.
  - Kept both `filters.recursion` and `recursionValues` sourced from the same derived list.

- Risks / Side effects
  - Low risk: recursion options now follow constants source-of-truth; behavior should match previous values. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Open Order Helper and verify Recursion filter options still show all three expected values and filter rows correctly.
...
- [x] In `src/orderHelperState.js`, import `ORDER_HELPER_TOGGLE_COLUMNS` from `./constants.js` and compute `schemaKeys = ORDER_HELPER_TOGGLE_COLUMNS.map(({ key }) => key)`.
- [x] Add a one-time validation inside `createOrderHelperState()`:
...
- [x] Define the canonical column defaults as:
...
- [x] When hydrating `storedColumns`, apply only keys in `schemaKeys` (ignore unknown stored keys), and always backfill missing schema keys to their canonical defaults.
- [x] Ensure `orderHelperState.columns` contains *all* `schemaKeys` so `applyOrderHelperColumnVisibility()` can consistently toggle CSS classes.
...
- What changed
  - Files changed: `src/orderHelperState.js`
  - Added schema/default key-set validation in `createOrderHelperState()` with `console.warn` output for mismatch details.
  - Added canonical column-default construction that backfills missing schema keys to `false`.
  - Restricted stored-columns hydration to schema keys only and always backfilled schema defaults.

- Risks / Side effects
  - In mismatched schema/default states, missing schema keys now default to hidden (`false`) and a warning is logged. (probability: ❗)
      - **🟥 MANUAL CHECK**: [ ] Open Order Helper, toggle several columns, reload the page, and confirm column visibility persists without console errors.
...
> Requires extensive analysis to prove there are no runtime mutation paths before introducing freezing behavior.
...
- [x] In `src/constants.js`, update the docstrings for `SORT.TITLE` and `SORT.ALPHABETICAL` to explicitly describe their effective sort field(s) as implemented in `sortEntries()` (currently `entry.comment` / “title/memo”, with `entry.key` fallback).
- [x] Add a short inline comment near `SORT.ALPHABETICAL` indicating whether it is a legacy/compatibility alias vs an intended UI option.
- [x] (Only if you confirm `SORT.ALPHABETICAL` is intentionally user-facing) Update `src/utils.js` `SORT_OPTIONS` to include a non-ambiguous label such as “Comment A‑Z / Z‑A”, and ensure it maps to `SORT.ALPHABETICAL` (declare this as a behavior change). Confirmed not currently user-facing in `src/utils.js`, so no `SORT_OPTIONS` change was applied.
...
- What changed
  - Files changed: `src/constants.js`
  - Updated `SORT.TITLE` docstring to match actual `sortEntries()` behavior (`entry.comment` with keyword fallback).
  - Added explicit legacy/compatibility alias note for `SORT.ALPHABETICAL`.
  - Confirmed `SORT.ALPHABETICAL` is not currently exposed in `src/utils.js` `SORT_OPTIONS`, so no UI option behavior was changed.

- Risks / Side effects
  - Documentation-only clarification; user-facing sort menus remain unchanged. (probability: ⭕)
      - **🟥 MANUAL CHECK**: [ ] Open sort dropdowns and confirm the same options as before are shown (no new `ALPHABETICAL` option added).
