...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `setEntrySelectionIcon(entry, isSelected)` near the top of the file.
- [ ] Replace the first copy (lines 15-17) with a call to `setEntrySelectionIcon(item, false)`.
- [ ] Replace the second copy (lines 30-32) with a call to `setEntrySelectionIcon(entry, true)`.
- [ ] Replace the third copy (lines 38-40) with a call to `setEntrySelectionIcon(entry, false)`.
...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `updateBookDropTargetState(book, shouldHighlight)` near the drop-target handlers.
- [ ] Replace the first copy (lines 175-182) with a call to the shared helper after the guard conditions are resolved.
- [ ] Replace the second copy (lines 186-191) with a call to the shared helper after the guard conditions are resolved.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ENTRY_SELECTION_ICON_SELECTOR = '.stwid--selector > .stwid--icon';`
- [ ] Replace each occurrence of the raw literal with `ENTRY_SELECTION_ICON_SELECTOR`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const UNSELECTED_ENTRY_ICON_CLASS = 'fa-square';`
- [ ] Replace each occurrence of the raw literal with `UNSELECTED_ENTRY_ICON_CLASS`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const DROP_TARGET_CLASS = 'stwid--state-target';`
- [ ] Replace each occurrence of the raw literal with `DROP_TARGET_CLASS`.
...
**Steps to fix:**
- [ ] Rename `item` to `selectedEntry` everywhere it appears in this callback.
...
**Steps to fix:**
- [ ] Rename `book` to `targetBookRow` everywhere it appears in the drop-target handlers in this file.
...
**Steps to fix:**
- [ ] Extract the selection UI helpers (lines 8-54) into a new function named `createSelectionStateHelpers()`. It should build the functions that add, remove, clear, and preserve selected entries.
- [ ] Extract the entry transfer flow (lines 56-172) into a new function named `createEntryTransferHandlers()`. It should own the move/copy logic and the SillyTavern event subscription cleanup.
- [ ] Extract the drag-drop handlers (lines 174-235) into a new function named `createBookDropHandlers()`. It should return the handlers for book, folder, and root drop targets.
- [ ] Replace the extracted blocks in `createSelectionDnDSlice` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the event subscription setup (lines 67-83) into a new function named `watchEntryTransferBooks(targetBookName, sourceBookName, isCopy)`. It should start watching the relevant lorebooks and return the cleanup pieces.
- [ ] Extract the per-entry transfer loop (lines 92-130) into a new function named `transferSelectedEntries(srcBook, dstBook, isCopy, hasConcurrentBookUpdate)`. It should copy entries, attempt source deletion when needed, and return the transfer result flags.
- [ ] Extract the save-and-notify phase (lines 136-161) into a new function named `finalizeEntryTransfer(result, sourceBookName, targetBookName, srcBook, dstBook, isCopy)`. It should save changed books, update runtime state, and handle partial-failure toasts.
- [ ] Replace the extracted blocks in `moveOrCopySelectedEntriesToBook` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the inner block (lines 112-128) into a new function named `tryDeleteTransferredSourceEntry(srcBook, uid)`. It should attempt the source deletion, clean up original-data state when deletion succeeds, and report whether the delete worked.
- [ ] Replace the inner block with a call to `tryDeleteTransferredSourceEntry(...)`.
