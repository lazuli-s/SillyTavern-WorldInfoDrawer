...
**Steps to fix:**
- [ ] Extract the shared save and rollback pattern into a new function named `runEntryStateSave(action)` near the top of the file.
- [ ] Replace the first copy (lines 150-171) with a call to `runEntryStateSave(...)`.
- [ ] Replace the second copy (lines 183-221) with a call to `runEntryStateSave(...)`.
...
**Steps to fix:**
- [ ] Rename `e` to `worldEntry` everywhere it appears in this file.
- [ ] If it is referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Rename `sel` to `selectorButton` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Rename `strat` to `strategySelect` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Extract selector UI creation (lines 36-107) into a new function named `buildEntrySelector()`. It should create the selector button and attach its click behavior.
- [ ] Extract body content creation (lines 108-123) into a new function named `buildEntryBody()`. It should render the comment and key text areas.
- [ ] Extract status control creation (lines 124-225) into a new function named `buildEntryStatusControls()`. It should build the enable toggle and strategy dropdown.
- [ ] Extract row-level click and drag listeners (lines 231-254) into a new function named `attachEntryInteractionHandlers()`. It should wire editor opening and drag preparation.
- [ ] Replace the extracted blocks in `renderEntry` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the inner block that builds the help toast list (lines 64-79) into a new function named `buildSelectionHelpToast()`. It should return the finished help list element.
- [ ] Extract the shift-range selection block (lines 46-58) into a new function named `selectEntryRange(startRow, endRow, entryList)`. It should add every entry in the range to the selection state.
- [ ] Replace the inner blocks with calls to the new functions.
...
**Steps to fix:**
- [ ] Remove the unused `evt` parameter from the click handler on line 232.
