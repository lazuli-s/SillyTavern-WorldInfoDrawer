...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `createDynamicOptionAccessors({ getValueForEntry, noneValue })` near the top of the file.
- [ ] Update the outlet helpers (lines 153-156) to use `createDynamicOptionAccessors(...)` instead of defining `getOutletOptions` and `getOutletValues` separately.
- [ ] Update the automation ID helpers (lines 165-168) to use `createDynamicOptionAccessors(...)` instead of defining `getAutomationIdOptions` and `getAutomationIdValues` separately.
- [ ] Update the group helpers (lines 179-182) to use `createDynamicOptionAccessors(...)` instead of defining `getGroupOptions` and `getGroupValues` separately.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ROW_SELECTED_CLASS = 'stwid--applySelected';`
- [ ] Replace each occurrence of the raw literal with `ROW_SELECTED_CLASS`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ICON_UNCHECKED_CLASS = 'fa-square';`
- [ ] Replace each occurrence of the raw literal with `ICON_UNCHECKED_CLASS`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ICON_CHECKED_CLASS = 'fa-square-check';`
- [ ] Replace each occurrence of the raw literal with `ICON_CHECKED_CLASS`.
...
**Steps to fix:**
- [ ] Rename `it` to `entryData` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Rename `v` to `optionValue` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Extract scope helpers (lines 41-48) into a new function named `createScopeHelpers()`. It should normalize and compare the scope values.
- [ ] Extract entry data helpers (lines 50-123) into a new function named `createEntryAccessors({ cache, getSelectedWorldInfo, sortEntries, entryManagerState })`. It should provide functions for retrieving and sorting entries.
- [ ] Extract dynamic option helpers (lines 125-182) into a new function named `createDynamicOptionHelpers({ getEntryManagerEntries, entryManagerState, isOutletPosition })`. It should provide outlet/automation/group options and value helpers.
- [ ] Extract preview and selection helpers (lines 184-251) into a new function named `createSelectionAndPreviewHelpers({ dom, entryManagerState })`. It should manage row selection and preview rendering.
- [ ] Extract filter wiring (lines 253-291) into a new function named `initEntryManagerFilters(...)`. It should call `createEntryManagerFilters(...)` and return the destructured helpers.
- [ ] Extract renderer wiring (lines 302-369) into a new function named `initEntryManagerRenderer(...)`. It should call `createEntryManagerRenderer(...)` and return `renderEntryManager`.
- [ ] Extract UI open/refresh functions (lines 371-402) into a new function named `createEntryManagerOpeners({ dom, entryManagerState, getCurrentEditor, getEditorPanelApi, renderEntryManager })`. It should return `{ openEntryManager, refreshEntryManagerScope }`.
- [ ] Replace the extracted blocks in `initEntryManager` with calls to the new functions and keep the final `return { openEntryManager, refreshEntryManagerScope };`.
...
**Steps to fix:**
- [ ] Extract the inner block (lines 86-91) into a new function named `getMaxDisplayIndex(entries)`. It should scan the entries and return the max numeric display index (or -1).
- [ ] Replace the nested `for` + `if` block with: `const maxIndex = getMaxDisplayIndex(entries);`
...
**Steps to fix:**
- [ ] Extract the inner block (lines 109-117) into a new function named `getEntryManagerRowData({ entryBook, row, cache })`. It should return either `{ book, dom, data }` or `null`.
- [ ] Replace the inline `map(tr => { ... })` block with a call to `getEntryManagerRowData(...)` to reduce nesting.
- [ ] Keep the `filter(Boolean)` so the final `source` list still removes null entries.