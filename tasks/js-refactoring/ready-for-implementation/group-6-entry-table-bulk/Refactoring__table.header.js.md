...
**Steps to fix:**
- [ ] Extract the shared "compute allValues, ensure defaults, compute isActive, toggle active class" logic into a new function named `updateFilterIndicatorState({ stateKey, stateValuesKey, getValues, normalizeFilters, entryManagerState, menuButton })` near the top of the file.
- [ ] Replace the first copy (lines 35-43) with a call to `updateFilterIndicatorState(...)`.
- [ ] Replace the second copy (lines 55-63) with a call to `updateFilterIndicatorState(...)`.
...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `ensureDefaultFiltersSelected({ stateKey, allValues, entryManagerState })` near the top of the file.
- [ ] Replace lines 36-40 with a call to `ensureDefaultFiltersSelected(...)`.
- [ ] Replace lines 45-48 with a call to `ensureDefaultFiltersSelected(...)`.
...
**Steps to fix:**
- [ ] Extract the shared "update indicator, apply, notify" steps into a new function named `applyFiltersAndNotify({ updateFilterIndicator, applyFilters, onFilterChange })` near the top of the file.
- [ ] Replace the first copy (lines 44-52) with a call to `applyFiltersAndNotify(...)` (keeping any mode-specific setup, like normalization, before the call).
- [ ] Replace the second copy (lines 64-69) with a call to `applyFiltersAndNotify(...)` (keeping any mode-specific setup, like normalization, before the call).
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ACTIVE_FILTER_CLASS = 'stwid--state-active';`
- [ ] Replace each occurrence of the raw literal with `ACTIVE_FILTER_CLASS`.
...
**Steps to fix:**
- [ ] Rename `titleDiv` to `headerTitle` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Rename `tr` to `headerRow` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Rename `th` to `headerCell` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Extract the initial DOM creation (lines 13-28) into a new function named `createFilterMenuShell()`. It should create and return `{ menuWrap, menuButton, menu }`.
- [ ] Extract the "create update functions for a given mode" logic (lines 30-70) into a new function named `createFilterMenuUpdaters({ stateKey, stateValuesKey, getValues, normalizeFilters, applyFilters, onFilterChange, entryManagerState, menuButton })`. It should return `{ updateFilterIndicator, updateFilters }`.
- [ ] Extract the "render options including the change handler" logic (lines 72-100) into a new function named `renderFilterMenuOptions({ options, stateKey, entryManagerState, updateFilters, menu })`. It should add the option elements to `menu`.
- [ ] Extract the final wiring and return (lines 102-106) into a new function named `finalizeFilterMenu({ menu, menuButton, menuWrap, updateFilterIndicator })`. It should call `wireMultiselectDropdown(...)`, append `menu`, and return `{ menuWrap, updateFilterIndicator }`.
- [ ] Replace the extracted blocks in `buildFilterMenu` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the inner "add or remove optionData.value from filters" logic (lines 85-92) into a new function named `toggleFilterValue({ stateKey, value, isChecked, entryManagerState })`. It should update `entryManagerState.filters[stateKey]` appropriately.
- [ ] Replace the nested block inside the event handler with a call to `toggleFilterValue({ stateKey, value: optionData.value, isChecked: inputControl.input.checked, entryManagerState })`.
- [ ] Keep the `updateFilters();` call (line 93) after the extracted function call.