...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `moveRowAndSyncCustomOrder({ row, direction, mode })` near `createMoveButton` (or near the top of `buildTableBody`).
- [ ] In the click handler (lines 217-232), replace the duplicated block with a call to `moveRowAndSyncCustomOrder({ row: tr, direction, mode: 'step' })`.
- [ ] In the dblclick handler (lines 241-252), replace the duplicated block with a call to `moveRowAndSyncCustomOrder({ row: tr, direction, mode: 'jump' })`.
- [ ] Inside `moveRowAndSyncCustomOrder`, keep the "filtered/visible rows" guard checks and keep the final `updateCustomOrderFromDom()` call in exactly one place.
...
**Steps to fix:**
- [ ] Extract a shared helper function near the top of the file named `buildTextInput({ name, tooltip, getValue, onChange })`.
- [ ] Make `buildTextInput` create the `<input>` element, apply common classes, set `name`, set tooltip, set `type = 'text'`, set initial value from `getValue()`, and attach a `change` handler that calls `onChange(nextValue)`.
- [ ] Replace the outlet/group/automationId input creation blocks with calls to `buildTextInput(...)`, leaving only the cell-specific code (for example: syncing filters, hiding the outlet wrapper, etc.) in the caller.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const TEXT_POLE_CLASS = 'text_pole';`
- [ ] Replace each occurrence of the raw literal `'text_pole'` with `TEXT_POLE_CLASS`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const STATE_FILTERED_CLASS = 'stwid--state-filtered';`
- [ ] Replace each occurrence of the raw literal `'stwid--state-filtered'` with `STATE_FILTERED_CLASS`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const ORDER_INPUT_TIGHT_CLASS = 'stwid--orderInputTight';`
- [ ] Replace each occurrence of the raw literal `'stwid--orderInputTight'` with `ORDER_INPUT_TIGHT_CLASS`.
...
**Steps to fix:**
- [ ] Rename `inp` to `inputEl` (or a purpose-based name like `numberInput`) everywhere it appears in `buildNumberInputCell`.
...
**Steps to fix:**
- [ ] Rename `e` to `entryRow` (or `entryInfo`) everywhere it appears in this function.
- [ ] If it is exported or referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Rename this `wrap` to something purpose-based like `entryCellWrap`.
- [ ] For other `wrap` variables in this function (for example: lines 423, 464, 580, 614, 642), rename them to purpose-based names like `outletWrap`, `groupWrap`, `recursionWrap`, `budgetWrap`, `characterFilterWrap`.
...
**Steps to fix:**
- [ ] Extract the "template lookup and validation" block (lines 102-108) into a new function named `getEntryEditTemplates()` that returns the required templates (or throws).
- [ ] Extract the sortable setup (lines 110-150) into a new function named `setupEntryManagerSorting({ tbody, dom, cache, enqueueSave, setEntryManagerSort, SORT, SORT_DIRECTION, getSortableDelay, $, getEntryManagerRows })`.
- [ ] Extract the row construction loop (starting at line 153) into a new function named `buildEntryManagerRow({ entryRow, dom, cache, enqueueSave, ... })` that returns the created `<tr>`.
- [ ] Inside `buildEntryManagerRow`, further extract per-column builders (for example: `buildSelectCell`, `buildMoveCell`, `buildEnabledCell`, `buildEntryCell`, `buildStrategyCell`, `buildPositionCell`, `buildOutletCell`, `buildGroupCell`, `buildRecursionCell`, `buildBudgetCell`, `buildCharacterFilterCell`).
- [ ] Replace the extracted blocks in `buildTableBody` with calls to the new functions, keeping `buildTableBody` as an orchestration function.
...
**Steps to fix:**
- [ ] Extract the button DOM creation (lines 202-209) into a new function named `buildMoveButton({ iconClass, tooltipText })` that returns the `<button>`.
- [ ] Extract the shared "move row and update custom order" logic (used by both click modes) into a new function named `moveRowAndSyncCustomOrder(...)` (see finding [1]).
- [ ] Replace the extracted blocks in `createMoveButton` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the inner timeout callback block (lines 218-232) into a new function named `moveRowByOneStepInFilteredList({ row, direction })`. One sentence: it moves the row up/down by one visible row and then syncs the saved custom order.
- [ ] Replace the body of the timeout callback with a call to `moveRowByOneStepInFilteredList(...)`.
...
**Steps to fix:**
- [ ] Extract the inner "create one checkbox row and wire its change handler" block into a new function named `buildRecursionOptionRow({ entryRow, tr, key, label, cache, enqueueSave, applyEntryManagerRecursionFilterToRow })`. One sentence: it builds and returns the `<label>` element for one recursion option.
- [ ] In `addCheckbox` (lines 582-602), replace the nested DOM-building block with a call to `buildRecursionOptionRow(...)` and append its result to `wrap`.
