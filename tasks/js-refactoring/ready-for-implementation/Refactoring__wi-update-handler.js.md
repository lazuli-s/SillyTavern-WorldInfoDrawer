...
**Steps to fix:**
- [ ] Extract the shared pattern into a new function named `maybeTriggerEditorRefreshForField({ isCurrentEditor, selector, expectedValue, triggerEditorRefreshOnce })` near the top of `updateWIChange`.
- [ ] Replace the `content` copy (lines 154-160) with a call to `maybeTriggerEditorRefreshForField(...)`.
- [ ] Replace the `comment` copy (lines 163-168) with a call to `maybeTriggerEditorRefreshForField(...)`.
- [ ] Replace the `key` copy (lines 173-178) with a call to `maybeTriggerEditorRefreshForField(...)` (with `expectedValue` pre-joined into a string).
- [ ] Replace the default copy (lines 199-204) with a call to `maybeTriggerEditorRefreshForField(...)`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const LOG_PREFIX = '[STWID]';`
- [ ] Replace each occurrence of the raw literal with `LOG_PREFIX`.
...
**Steps to fix:**
- [ ] At the top of the file (after imports), add: `const EDITOR_ROOT_SELECTOR = '.stwid--editor';`
- [ ] Replace each occurrence of the raw literal with `EDITOR_ROOT_SELECTOR` (for example: ``document.querySelector(`${EDITOR_ROOT_SELECTOR} [name="comment"]`)``).
...
**Steps to fix:**
- [ ] Rename `a` to `entryToInsert` everywhere it appears in this file.
...
**Steps to fix:**
- [ ] Rename `e` to `entryUid` everywhere it appears in this file.
- [ ] If it is exported or referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Rename `o` to `cachedEntry` everywhere it appears in this file.
- [ ] If it is exported or referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Rename `n` to `updatedEntry` everywhere it appears in this file.
- [ ] If it is exported or referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Rename each `inp` variable to `editorFieldInput` everywhere it appears in this file.
- [ ] If it is exported or referenced in other files, search for it project-wide before renaming.
...
**Steps to fix:**
- [ ] Extract the "World Info event wiring" (lines 319-330) into a new function named `registerWorldInfoListeners({ eventBus, eventTypes, onWorldInfoUpdated, onWorldInfoSettingsUpdated })`. It should attach the listeners and return a cleanup function.
- [ ] Extract the "editor duplicate refresh worker" logic (lines 275-299) into a new function named `createEditorDuplicateRefreshWorker({ getCurrentEditor, refreshList, reopenEditorEntry, waitForWorldInfoUpdateWithTimeout })`. It should return `{ queueEditorDuplicateRefresh }`.
- [ ] Extract the "wait for update" helpers (lines 237-265) into a new function named `createWorldInfoUpdateWaiter({ delay, createDeferred })`. It should return `{ waitForWorldInfoUpdate, waitForWorldInfoUpdateWithTimeout }`.
- [ ] Replace the extracted blocks in `initWIUpdateHandler` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the "remove books no longer in `world_names`" block (lines 86-92) into a new function named `removeStaleCachedBooks({ cache, worldNames })`. It should remove DOM nodes and delete cache entries.
- [ ] Extract the "load and render missing books" block (lines 94-101) into a new function named `renderMissingBooks({ cache, worldNames, loadWorldInfo, listPanelApi })`. It should ensure every `worldName` is present in `cache`.
- [ ] Extract the "sync entries (remove missing, add new, update existing)" block (lines 113-225) into a new function named `syncBookEntriesAndDom({ bookName, cache, data, listPanelApi, editorPanelApi, getCurrentEditor, setCurrentEditor })`.
- [ ] Replace the extracted blocks in `updateWIChange` with calls to the new functions.
...
**Steps to fix:**
- [ ] Extract the inner block that handles per-field differences (lines 147-208) into a new function named `applyEntryFieldDiff({ bookName, entryUid, oldEntry, updatedEntry, editorPanelApi, triggerEditorRefreshOnce })`. One sentence on what it does: it compares one entry's old/new field values, updates the entry row DOM, and decides whether the editor needs to refresh.
- [ ] Replace the inner block with a call to `applyEntryFieldDiff(...)`.
- [ ] Consider reducing nesting further by using early `continue` statements inside `applyEntryFieldDiff` (for example, skip work when `oldValue` equals `newValue`).
